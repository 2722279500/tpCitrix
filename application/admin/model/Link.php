...
class link extends Model
{
	...
	public function add()
    {
        ...
        $data['name'] = $post['name'];
        $data['path_url'] = $post['path_url'];
        $data['thumb'] = empty($post['thumb'])?0:$post['thumb'];
        $data['create_time'] = time();
        if(true == Db::name("sys_link")->insert($data))
        {
    ...
    public function edit()
    {
        ...
        if(true == Db::name("sys_link")->where(['id'=>$post['id']])->update($data))
        {
            exit(json_encode(['code'=>1,'msg'=>'操作成功']));
        } 
        exit(json_encode(['code'=>0,'msg'=>'操作失败']));
    ...
    {
        $dataInfo = Db::name("sys_link")->where(['id'=>$id])->find();
        return $dataInfo;
    }
    /**
     * [infoDelete 删除]
    ...
    {
        if(true == Db::name("sys_link")->where(['id'=>$id])->delete())
        {
    ...
    public function batchDelete($delArray)
    {
        ...
        {
            Db::name("sys_link")->where(['id'=>$delArray[$k1]])->delete();
        }
        exit(json_encode(['code'=>1,'msg'=>'删除成功']));
    }
    /**
